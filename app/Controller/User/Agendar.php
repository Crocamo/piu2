...
class Agendar extends Page
{
    ...
    private static function getAgenda($request, $disp)
    {
        $help = explode('_', $disp);
        $servicoID =   $help[1]; //id do serviço escolhido
        $ReqData    =   $help[0]; //data escolhida 2023-11-10
        ...
        $itens = View::render('user/modules/agendar/agenda', [
            'itensAgendados' => self::getItensAgendados($request, $servicoID, $ReqData),
            'date'           => $data
        ]);
    ...
    private static function getItensAgendados($request, $servicoID, $ReqData)
    {
        $itens = '';
    ...
    /* funcionando sem horario do de trabalho
    private static function getItensAgendados($request, $servicoID, $ReqData)
    {
        $itens = '';
    ...
    private static function getBox($box)
    {
        $content = View::render('user/modules/agendar/box', [
            'filtro'     => 'Serviço',
            'filtroName' => 'Servico',
            'option'    => self::getOption($box),
            'onchange'  => ''
        ]);
        return $content;
    }
     ...
     */
    private static function getOption($box)
    {
        echo '$box<pre>';
   print_r($box);
   echo '</pre>';  
